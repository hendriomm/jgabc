...
var selDay, selTempus = '',
  selPropers, selOrdinaries = {},
  selCustom = {},
  sel = {
    tractus: {},
    offertorium: {},
...
$(function() {
  ...
  function runGabcReplaces(gabc, gabcHeader) {
    ...
    if (match && match.length > 1 && !gabc.match(/<sp>R\/<\/sp>\.?/)) {
      var count = 2;
      gabc = gabc.replace(/<sp>V\/<\/sp>(?! \d)/g, function(match) {
        return match + ' ' + (count++) + '.'
      });
    }
    if (gabcHeader && /Sequentia|Hymnus/.exec(gabcHeader.officePart) && !gabc.match(/\b\d+\. /)) {
      var count = 2;
      gabc = gabc.replace(/\(::\)\s+(?=.*\([a-m]\))(?!\d|<sp>|A\([^)]+\)men\.\([^)]+\))/g, function(match) {
        return match + (count++) + '. '
      });
    }
    return gabc;
  ...
  var updatePart = function(part, ordinaryName) {
    ...
    if (isNovus && !isOrdinaryPart) {
      ...
      if (match) {
        id = litanyMap[match[1]].map(function(l) {
          return 'litanies/' + l
        });
        ...
        if (!isOrdinaryPart) {
          ...
          if ((sel[part].isAlleluia = isAlleluia(part, plaintext))) {
            ...
            if (isNovus) {
              ...
              var gradualeIsFirstAlleluia = ((sel.graduale && sel.graduale.id in chantID.alleluiaById) || isAlleluia('graduale', (sel.graduale.lines || [
                []
              ])[0][0])) && !/non\s+rep[eé]titur/i.exec((sel.graduale.lines || [
                []
              ])[0][1]);
...
<div id="divGraduale$num" part="graduale$num" class="multiple-graduales-$num">\
  ...
  function makeRubric(rubric, extraClass) {
    ...
    return rubric.reduce(function(jq, rubric) {
      return (jq.add($('<div>').addClass(classes).html(
        rubric.replace(/>/g, '</span>').replace(/<(?!\/?\w+>)/g, '<span class="quote">').replace(/([vr])\/./g, "<span class='versiculum'>$1</span>").replace(/[\[\]{}()]/g, "<span class='bracket'>$&</span>"))))
    }, $());
  }
...
<div class='block right'>\
  ...
  var shiftGabcForClefChange = function(gabc, newClef, clef) {
    ...
    if (newClef[0] == clef[0]) {
      if (clefI == 4 && baseClefI == 2) {
        ...
        if (swap.clefShift == 0) {
          ...
          gabc = gabc.map(function(gabc) {
            return gabc.replace(/[a-m]x/g, '');
          });
    ...
    var pitches = [].concat.apply([], gabc.map(function(g) {
      return g.match(/[a-m]/gi);
    })).map(function(letter) {
      return parseInt(letter, 23) - 10
    });
  ...
  var getPsalmToneForPart = function(part, tone) {
    ...
    if (!gTermination) {
      if (!(gTermination = tone.terminations[termination])) {
        for (i in tone.terminations) {
          gTermination = tone.terminations[i];
          break;
        }
      }
    }
    ...
    if (isAl) {
      if (sel[part].style == 'psalm-tone1') {
        ...
        if (part == 'alleluia') lines[i] = lines[i].replace(/([\.!?:;,]?)\s*$/, function(m, a) {
          return ', Allelúia' + a;
        });
        var line = lines[0];
        ...
        if (line.match(/ij|bis/)) {
          if (part.match(/^graduale/) && sel['alleluia'].style == 'psalm-tone1') {
            lines[i] = lines[i].replace(/([\.!?:;,]?)\s*$/, function(m, a) {
              return ', Allelúia' + a;
            });
          }
  ...
  var firstClassSaints = saintKeys.filter(function(saint) {
    return saint.class == 1;
  }).reduce(function(result, saint) {
  ...
  i = j
  var beginningOfYearEntry = {
    title: "(Principium Annis)",
  ...
  $.each(['','asperges','custom2','offertorium','custom4','custom5','custom6', 'agnus','custom8','custom9','custom10','ite','custom12'], function(i, key) {
  ...
  function selStyleChanged(e) {
    ...
    if (this.id == 'selStyle') {
      ...
      $('select[id^=selStyle]:not(#selStyle)').each(function(i, o) {
        if (baseStyle != o.value.slice(0, baseStyle.length)) {
          baseStyle = 'mixed';
          return false;
        }
      });
  ...
  var getAllGabc = window.getAllGabc = function() {
    ...
    var processSolesmes = shouldRemoveSolesmes ? removeSolesmes : function(gabc) {
      return gabc;
    };
    $('[part]').each(function() {
      var $this = $(this),
      ...
      gabc = header + processSolesmes(gabc.slice(header.original.length).replace(/\^/g, ''). // get rid of exsurge specific ^
        ...
        replace(/!\)/g, ')').replace(/\b([arv]\/)\./ig, '<sp>$1</sp>'). // versicle and response symbols
  ...
  $('ul.dropdown-menu').on('click', 'a[id^=include]', function(e) {
    ...
    var hasPageBreakBefore = $('#dropdown-menu-include .toggle-page-break').map(function() {
      return $(this).hasClass('has-page-break-before');
    }).toArray();
    var allPageBreaks = hasPageBreakBefore.reduce(function(a, b) {
      return a === b ? b : 0;
    });
  ...
  function updateStoredPageBreaks() {
    pageBreaks = $('#dropdown-menu-include .toggle-page-break.has-page-break-before').parent().map(function() {
      return this.id.slice(7);
    }).toArray();
    localStorage.pageBreaks = pageBreaks.join(',');
    $('div[part]').removeClass('page-break-before');
    $(pageBreaks.map(function(part) {
      return 'div[part=' + part.toLowerCase() + ']'
    }).join(',')).addClass('page-break-before');
  }
  ...
  $('body').on('click', 'a.toggleShowGabc', function(e) {
    ...
    if (this.checked) {
      ...
      $.when.apply($, parseRef(ref).map(function(ref) {
        ...
        var lines = [].concat.apply([], arguments).map(function(l) {
          return l.replace(/^\d+[a-z]*\.\s+/, '');
        });
      ...
      case 'toggleBarAfter':
        ...
        if (bar) {
          ...
          if (e.data.after == 'note') {
            var notes = note.neume.mapping.notations.reduce(function(a, b) {
  ...
  $(document).on('change', '.selectShowLectionem', function(e) {
    ...
    var selector = val.split(',').map(function(val) {
      return '.lectio-' + val;
    }).join(',');
...
[part^=custom] text[source-index]:not(.dropCap)', function(e) {
    ...
    if (showingGabc) {
      ...
      showToolbarForNote(this, editorialChange, {
        part: part
      });
    }
  });
